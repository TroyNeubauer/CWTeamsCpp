...
namespace CWTeams
{
	namespace ExcelUtils
	{
		...
		xlnt::cell GetCell(const xlnt::worksheet& sheet, xlnt::row_t row, xlnt::column_t col)
		{
			for (const auto& rows : sheet)
			{
				if (rows.front().row() != row) continue;
			...
			CW_FATAL("Failed to find cell [row: {}, col: {}] in sheet ", row, col.index, sheet.title());

			/*if (sheet.has_cell(xlnt::cell_reference(colIndex, rowIndex)))
			{
				CW_FATAL("Failed to find cell [row: {}, col: {}] in sheet ", rowIndex, colIndex.index, sheet.title());
			}
